...
namespace SigesoftWebUI.Models
{
    ...
    public class Api
    {
        ...
        private string MakeRequest(Uri url, HttpVerb httpVerb, Dictionary<string, string> args)
        {
            ...
            try
            {
                using (WebResponse response
                        = request.GetResponse() as WebResponse)
                {
                    StreamReader reader
        ...
        private string MakeRequest_(Uri url, HttpVerb httpVerb, Dictionary<string, string> args)
        {
            if (args != null && args.Keys.Count > 0 && httpVerb == HttpVerb.GET)
            {
                url = new Uri(url.ToString() + EncodeDictionary(args, true));
        ...
        public HttpResponseMessage rspCliente(string apiUrl, string metodoWebservice, object entrada)
        {
            var inputJson = JsonConvert.SerializeObject(entrada);
            ...
            HttpResponseMessage response = client.PostAsync(apiUrl + metodoWebservice, inputContent).Result;
